...
float Process::CpuUtilization() const {
  ...
  long processTimeSpent =
      LinuxParser::ActiveJiffies(pid_) / sysconf(_SC_CLK_TCK);

  // Calculate total system elapsed time after the process started
  // Formula: total system elapsed time = total system runtime - time when
  // process started
  long systemElapsedTimeProcessStarted =
      LinuxParser::UpTime() - LinuxParser::UpTime(pid_);

  // Formula: 100 * (total time spent in process / total time spent of system
  // since process started)
  cpuUtilization =
      ((float)processTimeSpent / (float)systemElapsedTimeProcessStarted);
